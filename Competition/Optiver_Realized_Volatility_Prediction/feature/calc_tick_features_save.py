...
def calc_tick_features(train, test, d_df):
    ...
    ma_list_0 = [5]
    mas_0 = [[pd.DataFrame(
        tick_df[tick_df.time_id.isin(list(d_df[range(ma)].loc[idx].values))].groupby('stock_id')[feats].quantile(
    ...
    mas_1 = [[pd.DataFrame(
        tick_df[tick_df.time_id.isin(list(d_df[range(ma)].loc[idx].values))].groupby('stock_id')[feats].agg(
    ...
    mas_2 = [[pd.DataFrame(
        tick_df[tick_df.time_id.isin(list(d_df[range(ma)].loc[idx].values))].groupby('stock_id')[feats].agg([np.mean]))
...
if __name__ == "__main__":
    data_path = "../data_init/"
    d_df_data = pd.read_pickle("../data_feature/d_df.pkl")
    train_data = pd.read_pickle("../data_feature/train_raw.pkl")
    test_data = pd.read_pickle("../data_feature/test_raw.pkl")
    train_data, test_data = calc_tick_features(train_data, test_data, d_df_data)
    train_data.to_pickle("../data_feature/train_tick.pkl")
    test_data.to_pickle("../data_feature/test_tick.pkl")
